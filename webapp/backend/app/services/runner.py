...
async def _run_pipeline(run_id: uuid.UUID) -> None:
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            result = await db.execute(select(Chunk).where(Chunk.project_id == run.project_id).order_by(Chunk.chunk_index))
            ...
            qdrant_store.upsert_chunks(
                ...
                payloads=[
                    {"chunk_id": str(c.id), "document_id": str(c.document_id), "page_number": c.page_number, "text": c.text}
                    for c in chunks
                ],
            )
            ...
            run.best_umap_params = {"n_neighbors": best["n_neighbors"], "min_dist": best["min_dist"], "score": best["score"]}
            run.status = "DONE"
