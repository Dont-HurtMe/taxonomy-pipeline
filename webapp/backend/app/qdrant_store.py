...
def search(project_id: uuid.UUID, query_vector: list[float], top_k: int = 10) -> list[qmodels.ScoredPoint]:
    name = _collection_name(project_id)
    client = get_client()
    if not client.collection_exists(name):
        return []
    return client.search(collection_name=name, query_vector=query_vector, limit=top_k)
